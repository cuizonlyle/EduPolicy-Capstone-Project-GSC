...
# Use Case
This project both frontend and backend is currently hosted using Azure Free Tier (F1) and the database is also hosted using FreeDB although limited yet this project is still accessible for testing
but there's a tiny issue such as that you cannot search any students data at the Marshall Interface using the hosted Website URL (https://gscedupolicyfront.azurewebsites.net) since this project has access to the school's
API that returns all the simple data of the students that exist such as (ID No., Fullname) and the school's API is protected by the Cloudflare and it treats the Azure request as a bot and does not return any data however
